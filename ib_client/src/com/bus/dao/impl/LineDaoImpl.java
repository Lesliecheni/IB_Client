...
public class LineDaoImpl implements LineDao{
	...
	public List<Line> find(Line condition) {
		...
		String sql = "select line.lineCode,lineName, case status when '0' then '禁用' else '启用' end,startLineTime,"
				+ "case direction when '0' then '下行' else '上行' end,stationName,line_station_ref.stationCode from line "
				    ...
				    + " where lineName like ? and  direction like ? and status like ?  order by line.lineCode, stationOrder";
	...
	public void updateLine(Line line) throws SQLException{
		...
		Connection connection = DaoFactory.getConnect();
		// 更新线路sql
		String sqlUpdateLine = "update line set lineName=?, status=?, startLineTime=?, direction=? where lineCode = ?";
		// 删除线路站点关联sql
		String sqlDelRef = "delete from line_station_ref where lineCode = ?";
		// 添加线路站点关联sql
		String sqlSaveRef = "insert into line_station_ref(lineCode, stationCode, stationOrder) values (?, ?, ?)";
	...
	public void saveLine(Line line) throws SQLException{
		...
		Connection connection = DaoFactory.getConnect();
		// 更新线路sql
		String sqlUpdateLine = "insert into line(lineName, status, startLineTime, direction) values (?, ?, ?, ?)";
		// 查询刚刚插入的线路code sql
