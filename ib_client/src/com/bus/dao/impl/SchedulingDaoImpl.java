...
public class SchedulingDaoImpl implements SchedulingDao {
	...
	public List<Scheduling> find(Scheduling condition) {
		...
		String sql = "select scheduling.code,line.lineCode,tcNumber,tcTime,scheduling.userCode,startStation,endStation,busLicense,"
					+ " startStationTab.stationName, endStationTab.stationName, line.lineName,sysuser.loginName "
					+ " from scheduling join station as startStationTab on startStation = startStationTab.stationCode"
					...
					+ " where busLicense like ? and line.lineName like ? and startStationTab.stationName like ? and endStationTab.stationName like ?";
	...
	public void updateScheduling(Scheduling scheduling) throws SQLException{
		Connection connection = DaoFactory.getConnect();
		String sql = "update scheduling set lineCode = ?,tcNumber = ? ,tcTime = ? ,userCode = ?,startStation = ?,endStation = ?,busLicense = ? where code = ?  ";
	...
	public void saveScheduling(Scheduling scheduling) throws SQLException{
		Connection connection = DaoFactory.getConnect();
		String sql = "insert into scheduling(lineCode,tcNumber,tcTime,userCode,startStation,endStation,busLicense) values(?,?,?,?,?,?,?)";
		PreparedStatement preparedStatement = connection.prepareStatement(sql);
		preparedStatement.setString(1, scheduling.getLineCode());
		preparedStatement.setString(2,scheduling.getTcNumber());
		preparedStatement.setString(3,scheduling.getTcTime());
