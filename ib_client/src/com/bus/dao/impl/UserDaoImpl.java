...
public class UserDaoImpl implements UserDao {
	...
	public void save(User user) {
		Connection connection = DaoFactory.getConnect();
		PreparedStatement preparedStatement = null;
		String sql = "insert into sysuser(loginName,password,name,phone,idCard,role,driving,status) values(?,?,?,?,?,?,?,?)";
		try {
			preparedStatement = connection.prepareStatement(sql);
	...
	public void update(User user) {
		Connection connection = DaoFactory.getConnect();
		PreparedStatement preparedStatement = null;
		String sql = "update sysuser set loginName = ?, name = ?, phone = ?, idCard = ?, role = ?, driving = ?, status = ? where code = ?";
		try {
			preparedStatement = connection.prepareStatement(sql);
			preparedStatement.setString(1, user.getLoginName());
