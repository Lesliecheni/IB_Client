...
public class BusDaoImpl implements BusDao {
	...
	public List<Bus> find(Bus condition) {
		...
		String sql = "select busCode,busLicense,busType,case busStatus when '1' then '启用' else '禁用' end,startTime "
				+ "from bus where busLicense like ? and busStatus like ?";
	...
	public void updateBus(Bus bus) throws SQLException{
		Connection connection = DaoFactory.getConnect();
		String sql = "update bus set busLicense = ?,busType = ?,busStatus = ?,startTime = ? where busCode =?";
